...
///////// for smart contract 
module.exports.getContracABI = async function(address) {
    return new Promise((resolve, reject) => {
        https.get('https://api-testnet.bscscan.com/api?module=contract&action=getabi&address=' + address + '&format=raw', (resp) => {
            let data = '';
...
module.exports.getTokenBalance = async function(chain, address, tokenAddress) {
    return new Promise(async(resolve, reject) => {
        var web3 = await BSC_HELPER.initChain(chain);
        var tokenABI = await BSC_HELPER.getContracABI(tokenAddress);
